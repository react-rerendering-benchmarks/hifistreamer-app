...
class Streaming extends Component {
   ...
   */
  getStatus() {
    fetch(getAPIurl() + "/streaming").then(response => response.json()).then(result => {
  ...
  onTIDALChange() {
    ...
    fetch(getAPIurl() + "/streaming?tidal=" + !this.state.TIDAL, requestOptions).then(response => response.json()).then(result => {
      this.setState({
        link: result.link
      });
      this.getStatus();
  ...
  render() {
    ...
    return <Form className="Page-text">
                <Form.Group as={Row} className="mt-4 ms-4">
                    ...
                    <Col>
                        <Form.Check type="switch" id="tidal-switch" checked={this.state.TIDAL} disabled={this.state.isDisabled} onChange={this.onTIDALChange.bind(this)}>
                        </Form.Check>
                    </Col>
                    <Row className="mt-2">
                        <Collapse in={this.state.isOpened}>
                                <div id="collapse-div" style={{
              width: '18em'
            }}>
                                    <div style={{
                display: 'flex',

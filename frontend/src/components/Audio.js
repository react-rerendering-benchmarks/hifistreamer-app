...
class Audio extends Component {
  ...
  getDeviceList() {
    fetch(getAPIurl() + "/audio").then(response => response.json()).then(result => {
      const options = result.devices.map(device => ({
        value: device.id,
        label: device.name
      }));
      this.setState({
  ...
  render() {
    return <Form>
                <Form.Group as={Row} className="mt-4 ms-4">
                    ...
                    <Col md="auto">
                        <Select type="device" className="basic-single" classNamePrefix="select" isClearable={false} onChange={this.onChangeDevice.bind(this)} value={this.state.selected} options={this.state.items}>
                        </Select>
                        <Button className="mt-4" variant="primary" disabled={this.state.isSaveDisabled} onClick={this.onSaveClicked.bind(this)}>
                            Save
                        </Button>                   
                    </Col>
                </Form.Group>
